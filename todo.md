...
### 最優先(システムの核心機能)
1. [x] **OpenAI連携改善** - character.chat APIに使用状況記録追加、トークン・コスト記録完了
2. [ ] **保護者画面実データ化** - weeklyData, skillData, recentActivitiesのAPI実装が必要

### 高優先度(ユーザー体験向上)
3. **管理者画面拡張** - グループ管理、リレーション設定、使用状況ダッシュボード
4. **テストユーザー設定** - 480030(生徒)、540011(保護者)、540022(講師)の関係設定

### 中優先度(バグ修正)
5. **キャラクター選択バグ** - 選び直し機能の確認と修正
6. **ログインボーナステキスト修正** - テキストとコメントの一致

### 低優先度(データ整備)
7. **storyChapters重複修正** - 同じタイトルのチャプター修正

---

## 新規バグ・機能要望(2025-10-29 Phase 4)

### 重大バグ
- [ ] **キャラクター選択バグ**: 仲間たちを一度選択すると選び直すことができない(コードは正常、UI確認待ち)
- [x] **宿題表示バグ**: StudentDashboard.tsxに宿題ボタン追加
- [x] **管理者ロールバグ**: StudentDashboard.tsxで管理者もアクセス可能に

### OpenAI連携改善
- [ ] **OpenAI使用状況連動**: チャット時のAPI呼び出しカウントが更新されない
- [ ] **チャットコンテクスト改善**: システムの目的(学習支援)を理解したプロンプト設定
- [ ] **リアルタイム音声会話**: OpenAI Realtime API対応

### 管理者画面拡張
- [ ] **グループ管理機能**: 子供、講師、親のグループを作成
- [ ] **リレーション設定**: 親-子供、講師-生徒の関係を管理画面で設定
- [ ] **使用状況ダッシュボード**: 各ユーザーの使用状況を把握

### データ整備
- [ ] **保護者画面の実データ化**: 仮データを完全に削除し、実データに置き換え

### 未完了タスク(Phase 3からの持ち越し)
- [ ] **ログインボーナステキスト修正**: テキストとコメントの一致
- [ ] **storyChapters重複修正**: 同じタイトルのチャプター修正
- [ ] **テストユーザー設定**: 480030(生徒)、540011(保護者)、540022(講師)の関係設定
