...
- [x] **アイテム画像が正しく表示されない問題を修正** (推定60分) - ✅ 完了
  - 対応: `getStudentItemsWithDetails`で`characterItems`と`gachaItems`の双方を参照し、既存データのIDずれを吸収
  - フォールバック画像とレガシーフラグを追加し、旧データでも正しい画像が表示されるようにした
  - 今後追加されるアイテムは従来通り`characterItems`を利用
