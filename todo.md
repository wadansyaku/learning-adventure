...
### 保護者画面改善
- [ ] **保護者-子供関係テーブル**: parent_childrenテーブル追加
- [ ] **実データ連携**: デモデータを削除し実データに切り替え
- [ ] **複数子供管理**: 1保護者が複数の子供を管理可能に

---

## 即座に修正すべき項目(優先度順)

1. **問題チャレンジの2問目バグ修正** (最優先・重大)
2. **ストーリークイズの送信ボタン追加** (最優先・重大)
3. **ロール切り替えの権限制御改善** (高優先度)
4. **問題画面UIの調整** (高優先度)
5. **保護者-子供関係システム実装** (高優先度)
