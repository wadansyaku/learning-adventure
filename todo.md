...
### Phase 0.5: 新規バグ修正と機能追加（優先度: 最高 - 即対応）

#### 0.5.1 プレビュー認証問題 ✅
- [x] **プレビューでログイン後の画面に移行できない** (完了)
  - 問題: プレビューでログイン後の画面に移行しようとしても、ログイン前の画面にしか進めない
  - 原因: 認証状態がプレビューで正しく伝播されていない可能性
  - 影響: プレビューでテストが困難
  - 修正: 開発環境でsameSiteを"lax"に変更、Partitioned属性を追加
  - ファイル: `server/_core/cookies.ts`

#### 0.5.2 帽子画像の追加 ✅
- [x] **35種類の帽子画像をガチャに追加** (完了)
  - 提供された画像: 35種類のPNG画像
  - 実装:
    1. PNG→WebPに変換（sharpライブラリ使用）
    2. S3にアップロード（manus-upload-fileコマンド使用）
    3. データベースに35個の帽子アイテムを追加
    4. レアリティ: Common(4), Uncommon(8), Rare(9), Epic(8), Legendary(6)
  - ファイル: `scripts/convert_hats.ts`, `scripts/upload_hats_to_s3.ts`, `drizzle/schema.ts`
