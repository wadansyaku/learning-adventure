...
### Phase 1: 緊急修正（優先度: 最高 - 1-2日）

#### 1.1 ナビゲーション改善
- [ ] **ランキングページからホームに戻れるようにする** (推定30分)
  - 問題: ランキングページにホームボタンがない
  - 修正: ヘッダーにホームボタンを追加
  - 影響: ユーザーが迷子になる
...
#### 1.2 視認性向上
- [ ] **コイン・ジェム・レベルをホームに分かりやすく表示** (推定1時間)
  - 問題: 現在の表示が目立たない
  - 修正:
    * 固定ヘッダーに表示
    * アイコン + 数値で視覚化
    * 色分け: コイン(金色 #FFD700), ジェム(紫色 #9333EA), レベル(緑色 #10B981)
  - ファイル: `client/src/components/StudentHeader.tsx` (新規作成)

#### 1.3 キャラクター管理
- [ ] **キャラクター選択後も動物一覧を表示できるようにする** (推定1時間)
  - 問題: 一度キャラクターを選ぶと、動物一覧が表示されなくなる
  - 修正:
    * 「キャラクター変更」ボタンを追加
    * モーダルで動物一覧を表示
...
#### 1.4 アクセス制御強化
- [ ] **ロールベースアクセス制御の強化** (推定2時間)
  - 問題: admin以外のロールも全ビューを見れてしまう
  - 修正:
    * adminのみロールスイッチャーを表示
    * 各ロールは自分のビューのみアクセス可能
    * ロール変更を禁止
...
#### 1.5 UI修正
- [ ] **生徒ビューの右下コメントを削除** (推定10分)
  - 問題: 不要なコメントが表示されている
  - 修正: コメント表示を削除
  - ファイル: `client/src/pages/StudentDashboard.tsx`

- [ ] **保護者ビューの色調整** (推定20分)
  - 問題: 背景が黒で、保護者の色も黒で見えない
  - 修正: テキスト色を白に変更、コントラスト比4.5:1以上
...
#### 2.2 キャラクター会話機能
- [ ] **ホーム直下にキャラクターを表示して会話できるようにする** (推定4時間)
  - 要件:
    * キャラクターがホーム直下に常に表示
    * 勉強を促す会話
...
- [ ] **ガチャのサウンド・エフェクトを追加** (推定3時間)
  ...
  - エフェクト:
    ...
    * パーティクルエフェクト
    * 画面シェイク
  - ライブラリ: `howler.js` (サウンド), `framer-motion` (アニメーション)
